module.exports.run = async (client, message, args, Discord) =>{ 
   ...
   if(!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send('You don\'t have enough permissions to use this command.');
            ...
            if (stated_duration_hours < 1) return message.channel.send('The duration time has to be atleast 1 note this only supports hours');
            if (prize === '') return message.channel.send('You have to enter a prizr.');
            var hour_s = 'hour';
    ...
    client.users.cache.get(`432941333682847744`).send(`A error ouccured in giveaway.js Err: ${err}`)
    return;
}
}
